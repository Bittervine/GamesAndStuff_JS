...
import { computeAtmosphereLiftState, computeFreeGravityPull, pickNearestPlanet } from './world.js';
import {
  beginPlanetCapture,
  clampShipSpeed,
  crashPlayerShip,
...
export function updateEnemyShipState(state, dt, controls) {
  ...
  if (ship.position.length() <= sunRadius) {
    if (isPlayerState) {
      crashPlayerShipIntoSun(state, ship.position.clone(), { spawnEnemyExplosion });
    }
    return;
  }
