...
function CheckResult() {
    ...
    if (brd_fiftyMove > 100) {
     ShowGameStatus("GAME DRAWN {fifty move rule}");      
     return BOOL.TRUE;
    }

    if (ThreeFoldRep() >= 2) {
     ShowGameStatus("GAME DRAWN {3-fold repetition}");      
     return BOOL.TRUE;
    }
    
    if (DrawMaterial() == BOOL.TRUE) {
     ShowGameStatus("GAME DRAWN {insufficient material to mate}");      
     return BOOL.TRUE;
    }
    ...
    if(InCheck == BOOL.TRUE)	{
        if(brd_side == COLOURS.WHITE) {
          ShowGameStatus("GAME OVER {black mates}");
          return BOOL.TRUE;          
        } else {
          ShowGameStatus("GAME OVER {white mates}");
          return BOOL.TRUE;          
        }
    } else {
      ShowGameStatus("GAME DRAWN {stalemate}");
      return BOOL.TRUE;      
    }	
...
function StartSearch() {
    ...
    srch_time = parseInt(tt);  
    var cn = $('#CNodesChoice').val();    
    srch_cnodes = parseInt(cn);  
    var ci = $('#CRandomChoice').val();    
    console.log("time:" + t + " TimeChoice:" + tt + "CNodes:"+ cn + "CIgnore:"+ci);
    srch_crandom = parseInt(ci)/1000.0; // From permil to probability
...
function NewGame() {
    ...
    srch_cmoves = 0; // New game, no moves done yet
}
