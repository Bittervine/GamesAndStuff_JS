...
Fixed in SDL build revision 139: Puppet Forge's combined X/Y/Angle mode displayed the selected key's easing but its apply path returned before writing anything. This made the selector appear to accept a change and then silently restore the old value when selection changed. Combined-transform easing now commits to all editable keys represented by the selected timeline diamond; constrained parts update their authored rotation key only.

Fixed in SDL build revision 140: the Level Editor's browser playtest correctly stored and applied the authored browser copy, but startup unconditionally showed the title screen and only auto-started explicit `?level=` or recording launches. Browser-copy playtests now join that same direct-start path after the level has loaded. The latest Raptor asset set is also promoted as one coherent atlas/rig/animation revision, including the separately articulated jaw and its smooth looping `easeInOut` rotation keys.

The same revision fixes a Puppet Forge export artifact exposed by the Raptor's constrained jaw. Parent-constraint baking used the clip duration as both an internal sampling boundary and an exported X/Y key time, so every constrained part in a looping animation could acquire an unexplained terminal-duration key. Looping bakes now still sample and adaptively refine the return interval to time zero, but omit the redundant endpoint from serialized tracks. Non-looping clips retain their ordinary final-duration key.

SDL build revision 142 introduces a deliberately small Development features submenu below Custom rendering. Both implementations expose Asset guides, Enemy guide, Game tuning, Recording, Playback, a compact Debug panel, and opt-in Debug logging. The SDL Asset guides draw the existing walkable/blockable geometry in green and yellow; Enemy guide draws enemy hitboxes, field-of-view cones, and current AI state. The SDL panel reports build/level, renderer or FIFO mode, pacing, tick, player, enemy, recording, playback, and logging summaries without a scrolling mini-log. Debug logging is disabled by default and writes one structured NDJSON snapshot per second only while enabled; the browser downloads its collected log when disabled, while SDL writes directly under `logs/`.

The same revision simplifies Windows iteration. `build.bat` now performs the ordinary CMake/MSBuild incremental build first and no longer touches every project source merely because the packaged revision changed. The executable revision check remains the safety net for ZIP files with misleading timestamps: only a failed check refreshes project timestamps and retries, with a clean rebuild retained as the final fallback. Runtime content, assets, reference modules, and development HTML are synchronized incrementally with standard `robocopy` behavior instead of deleting and recopying the complete staged content tree. Deleting `build\` remains the straightforward manual full-rebuild path.

SDL build revision 143 fixes raw SDL_GPU scene loss across fullscreen and window-size transitions. The FIFO producer previously acquired a swapchain target at the old size, then allowed render-time scene-texture recreation to cancel that acquisition and treated the resulting missing target as fatal. Scene sizing now uses an event-driven dirty flag, rebuilds before acquisition, skips stale queued presentation on transition frames, and reacquires once when Windows completes the resize one event later. Hidden steady-state cost is one cached boolean branch; SDL size queries and texture recreation occur only after relevant window events.

SDL build revision 144 fixes Development-feature playback launched through the native file picker. SDL permits the asynchronous file-dialog callback to run on an operating-system worker thread, but revision 143 performed level reload, renderer-resource replacement, menu mutation, and logging directly inside that callback. On Windows this could race the presentation thread, blacken the window, truncate the active NDJSON write, and hang during the second level load. The callback now copies its temporary filename and dispatches an owned result through `SDL_RunOnMainThread`; only the main-thread callback may start playback or mutate runtime/UI state.
...
Revision 393 extends the modular human atlas with split upper/lower arm and leg parts plus separate feet, then assembles `enemy_032` / **Human Raider III** from the existing Enemy 031 body/head selection. The new rig keeps the enlarged Human Raider gameplay contract but replaces the monolithic arm/leg artwork with articulated segments so future elbow, knee, and ankle animation can happen without tearing gaps.

Because the gameplay renderer still consumes explicit X/Y tracks, the new lower-arm, lower-leg, foot, and weapon positions are authored as baked parent-constraint results in dedicated `ct_anim_enemy_032_*` clips. Puppet Forge, the runtime fallback list, the enemy catalog, and the dormant level-generator metadata all now expose Enemy 032.

## Revision 394 corrected Enemy 032 anatomy and articulated walk

Revision 394 adopts the user-corrected `ct_rig_enemy_032.json`, `ct_anim_enemy_032_idle.json`, atlas manifest, character definition, and enemy-catalog entry as authoritative. The earlier Enemy 032 logical side names were exactly reversed, so every Enemy 032 upper arm, lower arm, upper leg, lower leg, foot, anchor, role, draw-order entry, parent reference, and animation track now swaps left and right. This is a clean data migration: no aliases or compatibility handling for the incorrect names remain.

Puppet Forge also retires the old “Left leg sprite” and “Right leg sprite” captions for `leftFoot` and `rightFoot`; they are now labelled **Left foot** and **Right foot**.

`ct_anim_enemy_032_walk.json` is rebuilt from the corrected idle stance and corrected parent-pivot rig. The two thighs alternate through contact and passing poses, the lower legs bend independently at the knees, the separate feet articulate through heel contact and toe-off, the free arm swings more broadly, and the sword arm remains comparatively controlled. Parent-linked X/Y motion is baked into the exported animation for the existing runtime. `devel/build_enemy_032_walk.mjs` makes the retarget repeatable after future Enemy 032 rig or idle changes.

The expanded human-atlas regeneration utility now ignores appended right-side components during its legacy connected-component scan and explicitly preserves the authored split-limb rectangles. This prevents an atlas rebuild from erasing the new articulated pieces.

## Revision 395 refreshed Enemy 032 combat clips
Revision 395 incorporates the user-updated `ct_rig_enemy_032.json`, `ct_anim_enemy_032_idle.json`, `ct_anim_enemy_032_walk.json`, `ct_atlas_enemy_030.json`, and `ct_char_enemy_032.json` as authoritative. Enemy 032's attack, hurt, and death clips are then rebuilt around that new idle and walk language so the corrected split limbs, feet, and weapon arm now move coherently across the full combat set.


## Revision 396 raised Enemy 032 combat baselines
Revision 396 keeps the user-authored Enemy 032 idle and walk authoritative and redoes the hurt, attack, and death clips so the taller split legs no longer pull the torso below the idle/walk baseline. Hurt now mostly reuses the idle stance with a compact upper-body jerk, attack keeps its sword-swipe idea while bracing from a raised body position, and death now begins from the same torso height as idle and walk before collapsing.


## Revision 397 subtle hurt and simulated Enemy 032 collapse
Revision 397 reduces Enemy 032's hurt response to a brief head-and-chest twitch with almost motionless legs. The death clip is now generated offline by `devel/simulate_enemy_032_ragdoll.mjs`: a deterministic 2D articulated collapse under gravity with a nearly invisible backward impulse, soft shoulder/elbow/hip/knee/neck limits, floor contact, and stiff ankles whose foot rotations follow their lower legs exactly. The resulting sampled motion is baked back into the normal animation JSON, so gameplay still uses the ordinary animation runtime rather than live physics.
...
Revision 400 keeps the production Enemy 032 death clip untouched but replaces the temporary showcase set again. The new `enemy-032-death-showcase.html` candidates are no longer just timing variations: each one now aims for a materially different final corpse silhouette, with the torso brought farther down, much larger separation in arm and leg end poses, and multiple supine/back-fall endings such as flat-on-back, one-knee-up, both-knees-bent, sprawled-leg, and sit-back-then-spill options. The goal of this pass is specifically to eliminate the bent-over hinge look seen in the previous showcase.

## Revision 401 twenty-four Enemy 032 death concepts
Revision 401 replaces the temporary Enemy 032 showcase set with twenty-four deliberately distinct authored concepts. Entries 1–12 follow the user-provided descriptions in order, including the heel-pivot log fall, Enemy 030 retarget, kneel-and-reach collapse, projectile lift, Goblin and Skeleton retargets, forward knee collapses, and stagger/trip variants. Entries 13–24 kitbash those ideas into additional combinations. The showcase uses joint-relative authoring, stiff ankle relationships, baked parent-pivot constraints, and a longer final-pose hold. The production `ct_anim_enemy_032_death.json` remains unchanged until a candidate is selected.

## Revision 402 floor-locked hybrid death showcase
Revision 402 keeps the 24 concept-driven Enemy 032 candidates and changes how their second halves are produced. Each authored animation now hands off to a deterministic ragdoll continuation at a reproducible pseudo-random point between one-third and two-thirds of its duration. The generated pose stream is floor-locked against the exact ground line measured from the starting idle pose, so no body part may sink below the level on which the enemy began standing. Ankles remain stiff and the temporary showcase reports each hand-off percentage. The production death clip remains unchanged.

## Revision 403 corrected hybrid-ragdoll gravity contact
Revision 403 diagnoses the apparent post-handoff anti-gravity in the Enemy 032 death showcase. Gravity was present at 500 units/s², but the old ground rule measured the deepest corner of every complete sprite rectangle, lifted the entire torso whenever an arm, foot, or sword crossed the standing floor line, added a repeated safety margin, and then gave the torso a small upward restitution velocity. The showcase generator now uses physical torso/head/foot support shapes and swept first-contact rollback. Gravity remains active, contact has zero vertical restitution, and no unrelated limb corner can hoist the whole character upward. The production death clip remains unchanged.

## Revision 404 promotes Enemy 032 death candidate 06
Revision 404 copies temporary showcase candidate **06 · Goblin-style collapse** into `ct_anim_enemy_032_death.json` as Enemy 032’s production death animation. The selected tracks are preserved exactly so the production clip can now be fine-tuned directly in Puppet Forge. The temporary 24-candidate showcase remains available as reference, but gameplay now resolves the ordinary `death` slot to candidate 06.
...
## Revision 503 visible-screen homing target boundary

Revision 503 fixes player rockets acquiring and retaining targets outside the current camera viewport. Homing-target candidate selection now requires the target object to intersect the visible world rectangle. Character-enemy targets use the enemy body rectangle so an enemy that is partly on screen remains eligible; point/debug targets use their authored target radius. An in-flight homing rocket releases a target once it leaves the screen and may reacquire another visible active target. Non-homing launch-time aiming remains unchanged.

## Revision 504 presentation-transform migration

Revision 504 performs the structural half of the micro-stutter fix without changing visible motion yet. Simulation-driven roots now own reusable `previousTransform`, `currentTransform`, and `shownTransform` records containing `x`, `y`, `angle`, `scaleX`, `scaleY`, and `alpha`. The fixed-step simulation reads and writes only `currentTransform`, snapshots it into `previousTransform` before every step, and copies `currentTransform` directly into `shownTransform` once per requested render frame. Canvas2D and WebGL presentation, presentation culling, actor artwork origins, shadows, moving visuals, and projectile orientation consume the shown transform. Character enemies also own reusable previous/current/shown animation-clock values so articulated pose sampling can be blended later without copying every limb pose.

Legacy root fields such as direct player/enemy/projectile `.x`, `.y`, `.angle`, `renderScale`, `renderOpacity`, and simulation-owned `animationTime` have been removed from migrated runtime objects. Dynamic moving-platform visuals similarly use transform triplets rather than direct position, rotation, and alpha fields. All copies mutate preallocated objects in place, so the migration adds fixed storage rather than per-frame garbage. Revision 504 deliberately performs no interpolation: shown equals current, and playtesting should therefore confirm identical behavior and visuals before smoothing is enabled.

The next planned revision is the small behavioral step: calculate one render blend fraction from the fixed-step accumulator and interpolate shown transforms and articulated animation clocks between previous and current, with shortest-path angle blending and snap rules for spawning, teleports, respawns, level transitions, visibility discontinuities, and scale-sign changes.

## Revision 505 fixed-step presentation interpolation

Revision 505 completes the micro-stutter fix prepared by revision 504. The browser keeps the deterministic 60 Hz accumulator simulation, calculates a clamped presentation blend from the remaining accumulator fraction, and asks `preparePresentationFrame` to interpolate the reusable shown records between previous and current. Position, scale, opacity, and simulation-driven articulated-enemy animation clocks blend linearly. Projectile and actor angles take the shortest wrapped route, while scale sign changes snap rather than squeezing a sprite through zero. Canvas2D and WebGL require no renderer-specific interpolation code because both already consume shown transforms.

Interpolation remains presentation-only: current transforms still own collision, AI, targeting, scripted movement, serialization, and tests. Level application, reset/respawn, development pose changes, camera reset, newly normalized spawns, and other discontinuities align previous/current/shown through the existing snap helpers. Paused development stepping presents current immediately instead of deliberately retaining the normal one-tick interpolation delay.

Implementation uncovered one stale transition hazard and fixed it immediately: an articulated enemy that changed animation slot reset only its current animation time, which would have blended the new clip from the old clip's previous time. Animation-slot changes now snap all three clock values to zero together. The in-game profiler button also records all frames with zero thresholds and includes `interpolationBlend`, allowing later captures to show that internal 0-step/2-step cadence can remain while displayed motion advances smoothly.


## Revision 506 tiled-bake diagnostic instrumentation

Revision 505 profiling separated two previously entangled symptoms. Presentation interpolation flattened the fixed-step `2, 0, 2` cadence even when baking was off, while the remaining noticeable bumps correlated with rolling WebGL tile-atlas updates. The baking-off control capture had no tile uploads or texture updates and felt smooth; the tiled capture showed periodic update bursts near the 256-pixel tile cadence. This remains a suspected tiled-renderer hitch rather than a proven GPU cause, so revision 506 deliberately makes no cache-policy or upload-order change.

The opt-in micro-stutter profiler now activates a low-disturbance tiled-bake diagnostic layer. It measures worker-result collection, completed-tile adoption, atlas-slot allocation, texture-update call time, cache planning, eviction, job scheduling, diagnostic bookkeeping, and baked-tile drawing. Per-frame counters record completed/adopted/uploaded tiles, upload bytes, pages and slots touched, evictions, queued/started/remaining jobs, record counts, worker state, upload burst identity, upload tile/page/slot coordinates, and the camera tile relationship. Summary totals group consecutive upload frames by burst ID.

The instrumentation is inactive outside an explicit profiler recording. It reuses preallocated renderer diagnostic records, performs no frame-time logging, and adds no `gl.finish`, readback, fence, or GPU timer query. The existing one-completed-tile-per-frame adoption limit, serial worker dispatch, prediction corridor, cache regions, texture budgets, atlas layout, eviction policy, and drawing order are unchanged. The periodic tiled-mode hitch remains open pending a controlled revision-506 recording.

## Revision 507 renderer-agnostic marked-stutter diagnostics

Revision 506 testing showed occasional visible hitches in WebGL with Tiles, WebGL with Full, WebGL with baking Off, and Canvas2D with baking Off. That weakens the tile-atlas and WebGL-specific explanations. The retained Full-bake capture also showed regular rAF timestamps, low JavaScript work, no texture uploads, and correctly compensated fixed-step interpolation during reported stutters. The remaining investigation therefore moves upstream to callback delivery and presentation motion without changing simulation, interpolation, renderer selection, baking policy, or cache behavior.

Revision 507 expands the profiler schema to record the actual `performance.now()` interval between callback entries separately from the browser-supplied rAF timestamp interval. It also records callback-entry lateness relative to the rAF timestamp, active renderer and baking mode on every sample, authoritative and shown player/camera positions, their frame-to-frame deltas, and the player's screen-relative movement after camera motion. Presentation snap helpers now maintain a diagnostic-only sequence, reason, subject, and kind so a reset, camera cut, level application, development pose change, or animation-clock snap can be correlated with the exact frame. This also fixes the profiler export's stale camera summary, which still read removed direct camera `.x/.y` fields instead of transform records.

The development tool strip adds **Mark stutter**. While profiling, pressing it preserves up to six seconds of prior samples and captures 120 frames after the mark (about two seconds at 60 Hz). The profiler then stops automatically and leaves a **Copy marked capture** action. The marker does not modify game timing or invoke GPU synchronization. All new position, callback, and snap diagnostics are collected only while profiling is active. The occasional renderer-independent hitch remains open pending a revision-507 marked capture.

## Revision 508 profiler-control cleanup and corrected hitch scope

Revision 507's **Mark stutter** control is removed. Its separate post-roll/automatic-stop workflow did not provide telemetry beyond the profiler's existing bounded all-frame ring, consumed development-tool-strip space, and could report a successful clipboard action while leaving the prior clipboard contents unchanged. The ordinary profiler toggle is now the only in-game workflow: click once to start, then click again promptly after seeing a hitch to stop and copy the latest 900 retained frames. Callback-entry timing, renderer/baking identification, current/shown player and camera motion, snap diagnostics, and tiled-bake counters remain unchanged.

The earlier camera-reversal discontinuity is explicitly not accepted as the reported micro-stutter. The user does not react to direction changes and observes the target symptom only during long continuous stretches while firmly holding one direction. Revision 508 therefore makes no camera, facing, look-ahead, input, physics, interpolation, rendering, or baking change. Subsequent analysis must isolate steady-direction frames around the captured hitch rather than selecting large but unrelated reversal motion.


## Revision 509 callback-arrival simulation clock and fresh-profile renderer defaults

Revision 509 changes the browser accumulator's elapsed-time source from the timestamp supplied by `requestAnimationFrame` to the actual `performance.now()` value sampled when the callback begins. The simulation remains a deterministic fixed 60 Hz loop, still executes at most eight catch-up steps per callback, and retains the existing presentation interpolation between reusable previous/current transform states. A single callback delta is clamped to 100 ms so tab stalls, debugger pauses, and operating-system suspensions cannot create an unbounded catch-up spiral.

The browser-supplied rAF timestamp is retained independently for profiler diagnostics. Captures can therefore compare the presentation timeline with actual JavaScript callback arrival, while `realDtMs`, accumulator growth, fixed-step count, and interpolation blend now reflect the time that genuinely elapsed between callback entries. This cannot manufacture a frame while JavaScript is not scheduled, but it prevents a late callback from advancing simulation by only the nominal rAF interval and lets the following early callback naturally contribute its shorter real interval.

Fresh profiles now default to hardware rendering enabled and baking Off. Existing stored settings remain authoritative, including explicit Canvas2D or baking choices. Electron continues to follow the same persisted setting rather than forcing WebGL.

## Revision 510 level foreground treatment and texture prewarming

Profiling a large fullscreen viewport exposed a concrete gameplay hitch unrelated to fixed-step timing: the first appearance of a cave-foreground treatment variant could synchronously build its filtered sprite canvas and upload that new canvas as a WebGL texture. One captured miss spent 21.5 ms in the foreground pass and another spent 16.3 ms, even though ordinary WebGL frames remained well below one millisecond. The existing level-presentation prewarm hook handled only dimmed background atlas variants, so the expensive foreground work still happened inside active play.

Revision 510 extends that loading-stage hook across every unique `caveForeground` atlas frame and brightness/saturation treatment referenced by the active level. Each treated sprite canvas is built once while the loading surface is visible, deduplicated by the same colour-map-aware key used by normal rendering, and retained in the ordinary foreground cache. WebGL additionally creates the derived textures before gameplay resumes through the backend's normal texture cache path, without GPU readback, `gl.finish`, or a renderer-specific visual alternative. Canvas2D benefits from the same CPU-side sprite preparation.
...
## Revision 145 consistent menu Back controls
...
Atlas 001 now exposes `horizontal_blocker` and `vertical_blocker` editor utilities from completely transparent unused atlas rectangles. Each utility uses a closed four-sided blockable collision loop and remains visible in the editor through collision guides rather than artwork. Explicit negative `paletteOrder` values place the utilities first in the all-atlas palette; no underscore prefix is used. Manually placed copies still respect the chosen placement layer, so they collide on Terrain and remain inert on Foreground or Background. The editor previews any fully transparent colliding frame by drawing its collision geometry. Invisible blocker `generationTags` are intentionally unconstrained: authors may tag them `capability.platform` so they can participate in signal-driven moving-platform setups, and regression tests must not require those tags to be empty.
...
## Revision 370 continuous-ground cleanup and durable saves

Status: implemented.

The revision-369 stride solver remains the fallback for blocked grounded travel, but continuous authored slopes no longer enter it merely because the same atlas placement has a filled blocking polygon beneath its top line. Both runtimes preserve support placement identity and ignore that sibling underfill only while the held support continues. Synthetic parity coverage requires smooth uphill motion with no stride-active frames.

Hunter graph recovery is broadened so projectile hunters can use the same same-floor local pursuit already available to melee hunters. Exact shared support identity is authoritative before Y-distance heuristics, which covers enemies and Ignatius standing far apart on the same authored incline. Visible `unreachable_glare` uses that local recovery immediately rather than waiting for another graph-repath interval. Perception and Magic Ring concealment are unchanged.

SDL settings and save slots move to the stable SDL preference directory, with safe one-time import of executable-local legacy settings/save files and executable-local fallback only when preference storage is unavailable. Diagnostic logs and recordings remain revision-local beside the executable.
### Build revision 384 test-fixture layer schema cleanup
