...
The former `rocketOverdrive` effect is now canonically `speedShot`; the shared normalizer still accepts the old ID for revision-211/212 snapshots. Speed Shot remains an independent thirty-second effect with HUD priority 100, half projectile fuel cost, and half launch cooldown. The inactive Power label is now simply `Powerup:`.
