...
Character-enemy lunge eligibility is authored data, not an enemy-ID branch. `meleeHitRange` owns the close melee distance when positive; if it is zero, close-attack eligibility falls back to `attackRange` and hit resolution remains on the legacy handoff-centered `meleeHitRadius`. `lungeRangeMin`/`lungeRangeMax` define the long-range commitment band; `lungeSpeed` is physical burst speed; and `lungeTargetDist` defines the desired locked separation at lunge launch. Melee hit reach and authored lunge target separation both scale with enemy instance scale in JS and SDL. `src/core/simulation.js` and `src/core/Simulation.cpp` perform the same full grounded path probe before commitment and again at launch, authored-speed wind-up, impact-pose hold, target lock, incrementally substepped collision-aware burst, delayed handoff hit test, and authored-speed follow-through. Once a burst is underway, only the movement actually attempted that tick is checked, so a newly introduced distant blocker does not freeze the enemy before contact while walls and gaps still stop the lunge without tunnelling. Zero `lungeRangeMax` keeps ordinary melee behavior, which is how non-eligible melee enemies such as the boxer remain non-lunging without species-specific code.
