...
Revision 167 adds `locomotion: "flying"` as an orthogonal movement qualifier rather than a new strategy. Portable simulation owns the deterministic horizontal patrol, vertical bobbing, target synchronization, and death fly-off distance. Flying enemies do not ground-snap, acquire moving-platform support IDs, or enter the support-graph navigation system. Their voluntary flight movement is nevertheless collision-aware against blockable world geometry, including shallow blocking lines, and moving solid geometry may physically shove or telefrag them. The Level Editor preserves the authored field and exempts these records from its automatic ground snap. Rendering only suppresses the ordinary ground shadow and presents the same character-project draw commands as every other enemy.
...
Named channels are plain portable broadcast data, not DOM events. `src/shared/signal-channel-data.js` normalizes stable channel names and lever/keyhole emitter records. `src/shared/moving-platform-data.js` stores the selected activation mode, channel, and respawn-persistence flag on a platform. `src/core/simulation.js` owns the channel revision counter, automatic lever/keyhole proximity-edge detection, entity-state transition, inventory mutation, optional key consumption, one-second emitter/receiver fades, end-of-fade entity/collision removal, and listener triggering. Each listener independently observes a newer channel revision, so one emission fans out to every gate, moving platform, or other target assigned to that channel rather than being consumed by a single slot. Levers re-arm only after Ignatius leaves their one-wizard-width proximity; keyholes are one-shot and are removed when their fade completes. Signal gates remain collision-bearing throughout their fade, then their runtime visual/entity/receiver/solid are removed atomically and are not resurrected by later channel toggles. The separately authored `proximitySignalTrigger` retains its own configurable trigger radius.
...
Historically, `devel/build_enemy_030_assets.py` could create missing fallback content while preserving authored clips. That pre-articulation atlas constructor is now retired: the current reforged human atlas, baked complexion arm frames, and articulated rigs are authoritative, and the obsolete script is marked `DELETE ME` so it cannot regenerate removed full-limb assets. `devel/retarget_enemy_030_animations.py` remains the explicit animation retargeting utility. Runtime collision remains separate from render geometry, and the Human Raider catalog entry owns its gameplay body.
...
The modular human contract now has a proven variant boundary. A visual variant receives a separate rig JSON whose authored atlas-frame selections may change while logical articulated part names, pivots, constraints, and animation tracks remain unchanged. Its character JSON may point directly to another human variant's animation files because animation data addresses rig roles (`torso`, `head`, `leftUpperArm`, `leftLowerArm`, and so on), not atlas frame IDs.

`devel/build_human_enemy_variant.py` is the current authoring utility for this boundary. It rejects missing or dimension-mismatched body/head frames, requires the articulated rig schema, selects baked `*_body_XX` arm artwork automatically when the chosen torso has it, otherwise selects the canonical fair-skin articulated arms, and records the resulting assembly in `ct_human_parts_030.json`. It never emits character-part `colorExchange`.


## Revision 371 character sprite Color Exchange boundary (historical; superseded)

Revision 371 temporarily allowed per-part character `colorExchange`. That character-specific feature is retired. Current character rigs must reference immutable authored atlas pixels and Puppet Forge plus both runtimes reject stale per-part `colorExchange` data. Human complexion variants are baked arm frames and the blue bat uses its own authored atlas.

The shared GEGL-compatible Color Exchange utilities remain current for **level-wide world-atlas treatment only**. They must not be reintroduced into character rig or character presentation data. The old browser sprite preprocessing module is marked `DELETE ME`.
...
The articulated human family uses a 600-pixel `maxFallDistance`. This is gameplay mobility data owned by the current enemy catalog; the retired pre-articulation `build_enemy_030_assets.py` must not regenerate it. Level 001 carries an exact rebake for that profile, including both the left jump to the starter platform and the right walk-off to the lower floor.
...
On Windows, `IgnatiusDevTool.exe` hosts the same shell in one WebView2 controller. The native bridge supplies the selected `authoringResourceRoot`, validates the same canonical directory/index shape as Chrome, performs typed writes and arbitrary text reads only below that root, updates the resource index under the same completeness rules, and launches playtests requested by the page. Blob/image/font/media reads use the dedicated `ignatius-project-resources.example` virtual origin mapped directly to that live root, so they cannot fall through to packaged `appassets.example` content. The top-level shell is the sole project-folder owner; embedded sub-tools inherit that host and do not create standalone folder-selection state. The native executable therefore retains filesystem and process privileges while no longer maintaining a duplicate four-tab user interface. Chrome and the SDL WebView2 host are the supported environments; other browser engines are not a compatibility target.
...
## Revision 364 Magic Ring perception boundary and disappearing gate collision ownership

Magic Ring concealment is an active shared power-up effect rather than an enemy-specific flag. `characterEnemyCanNoticePlayer` is the ordinary perception boundary and rejects the player while `magicRing` is active, but projectile target availability is intentionally left unchanged so homing projectiles continue tracking Ignatius. Damage from the concealed player starts or refreshes per-enemy panic state for ten seconds without populating last-seen player coordinates. Panic uses deterministic enemy-local choices for short movement/attack phases, keeping browser/native simulation reproducible while still producing arbitrary-looking fire directions. Player presentation carries a separate brightness multiplier of 0.5; alpha remains 1.0.

Signal-disappearing entities share a 0.75-second fade. Gate collision ownership includes both the synthetic signal receiver solid and atlas-manifest segments/polygons whose `visualId` belongs to the gate entity's visuals. Those records remain live during the fade, then are removed in the same completion step as the runtime visual/entity. Browser removal replaces collision arrays so the world index observes a new source identity; native removal advances `collisionRevision`. Any player or character enemy whose support ID belongs to removed geometry is detached immediately. This keeps the visible fade physically solid but prevents an invisible post-fade collision shell.

## Revision 369 grounded discontinuity and stride-arc solver

Player ground locomotion has two deliberately separate paths. Clean grounded horizontal motion follows its current/continuous support without running a stride search. If forward travel is obstructed, the portable simulation invokes a fixed-reach foot solver whose radius is `player.height * 0.20`.

The stride broad phase uses the player collision box at the blocking contact, expanded by the complete reach. Atlas placements are broad-phase units: once a collidable placement overlaps that query, all of its authored collision lines enter the narrow phase, along with relevant direct segments, solids, and collision polygons. The leading foot on the currently held support is the circle origin; no riser identity is part of the solver contract.

The movement-facing semicircle is swept from its top toward its bottom. The first exact standable circle contact is a foothold. If the first exact contact is steep/blocking, the contact is glided inward along that edge toward the foot origin until a reachable standable intersection is found; failure leaves ordinary movement blocked. A disconnected standable line endpoint can trigger this process without any riser, but its exposed endpoint must itself be inside the fixed reach.

A planned stride remains persistent and consumes the same distance budget as ordinary locomotion. The lower one-fifth-height player band is the foot/step envelope; above it, each stride leg is validated with the continuous swept convex hull of the player body so ceilings and taller secondary obstacles cannot be tunneled through. Browser and native implementations share these exact constants and ordering.

## Revision 370 slope continuity, local hunter recovery, and durable SDL persistence

The automatic player stride is entered only for a genuine grounded discontinuity. Atlas collision commonly represents one platform with both a shallow top line and a closed blocking polygon underneath it. While the player is holding that top line and it still has a valid Y at the proposed horizontal X within the normal automatic-step tolerance, the sibling polygon with the same placement `visualId` is treated as underfill rather than a new obstacle. This prevents a continuous incline from alternately following the support and starting persistent strides. Once the held line ends, or when a polygon belongs to another placement, ordinary blocking/stride discovery resumes unchanged. Support-geometry records therefore retain `visualId` in both runtimes, including polygon-held supports.

Hunter local pursuit is a physical-floor fallback beneath the navigation graph. It is not melee-specific: projectile hunters can use it after graph failure too. Exact shared support identity (`player.supportId` matching the enemy support/current support) is authoritative same-floor evidence and is evaluated before vertical-distance heuristics, so a long incline cannot be misclassified merely because its two occupants differ substantially in Y. `stranded_patrol` already tries the local path on a fresh sighting; `unreachable_glare` now does so immediately on fresh sight before either waiting for its graph-repath timer or planning another graph route. The fallback establishes `pursue`, clears the failed route, attacks if currently valid, otherwise moves toward an appropriate local distance, and emits `ENEMY_REENGAGED_LOCAL`. It is reached only after normal fresh perception has succeeded, so Magic Ring concealment and awareness/facing remain unchanged.

SDL persistence now separates durable player data from revision-local diagnostics. `SDL_GetPrefPath("Bittervine", "IgnatiusRocketfrock")` owns settings and save-slot JSON so extracting a new build directory does not create a blank save namespace. Startup initializes that directory before reading settings/saves and copies legacy executable-local `userdata/settings.json` plus `userdata/save_*.json` into it only when the target file does not already exist. Executable-local `userdata` is retained only if SDL cannot provide a preference directory. Logs and recordings deliberately stay next to the executable, preserving the existing diagnostic containment model.

## Revision 432 committed melee-lunge contract

Character-enemy lunge eligibility is authored data, not an enemy-ID branch. `meleeHitRange` owns close melee reach and is measured horizontally from the enemy grounded-base X; active melee definitions author a positive value explicitly and never borrow projectile-only `attackRange`. The melee handoff owns impact timing and strike Y. `lungeRangeMin`/`lungeRangeMax` define the long-range commitment band; `lungeSpeed` is physical burst speed; and an explicit positive `lungeTargetDist` defines the desired locked separation at lunge launch. A zero target distance disables lunging rather than deriving a percentage of melee reach. Melee hit reach and authored lunge target separation both scale with enemy instance scale in JS and SDL. `src/core/simulation.js` and `src/core/Simulation.cpp` perform the same full grounded path probe before commitment and again at launch, authored-speed wind-up, impact-pose hold, target lock, incrementally substepped collision-aware burst, delayed handoff hit test, and authored-speed follow-through. Once a burst is underway, only the movement actually attempted that tick is checked, so a newly introduced distant blocker does not freeze the enemy before contact while walls and gaps still stop the lunge without tunnelling. Zero `lungeRangeMax`, zero `lungeSpeed`, or zero `lungeTargetDist` keeps ordinary melee behavior without species-specific code.

Revision 432 supersedes Revision 375's `attackLungeDistance` / `attackLungeSpeed` tracking-nudge contract. Those retired fields and global defaults are no longer runtime inputs. The retained `devel/simulate_melee_lunge_balance.py` script is a retired historical experiment. It remains in-tree for reference only and is not a source of gameplay tuning or verification.
...
## Revision 449 test-gate execution tiers

Ordinary Linux builds now default to Release, matching the Windows build helper; Debug remains an explicit diagnostic choice. The native end-to-end character resource regression loads one representative projectile enemy through project, rig, atlas, animation, image, texture, projectile-palette, and sound resolution. Catalog-wide authored-data coverage remains in the dependency-light browser resource/schema gates instead of repeatedly decoding every atlas in the native suite.

The ordinary browser release gate keeps every generator theme/configuration assertion but uses two deterministic seeds per configuration. `npm run test:stress` restores the former exploratory seed counts and full theme-by-length matrix for deliberate broad sweeps. Resident-memory sampling is profiling metadata only and falls back to zero when the host cannot expose process RSS; it never controls test success.

## Revision 450 release-gate coverage boundaries

The generator compatibility contract treats theme, route length, route implementation, and cavern implementation as independent supported inputs. The ordinary release gate therefore executes their complete 2 x 2 x 2 x 2 cross-product. Only additional deterministic seeds are delegated to `npm run test:stress`; configuration dimensions are not sampled away as though they were redundant seeds.

Profiling uses current process RSS when available and otherwise labels Node's `resourceUsage().maxRSS` as a peak-only sample. If neither source is available, output says `RSS unavailable`. Memory samples remain diagnostic and cannot affect assertions.

Native character coverage stays split by cost: one representative real enemy exercises resource loading, while a two-part synthetic project with a single 8 x 8 in-memory texture exercises the parent-constrained draw path. This preserves the behavioral contract without decoding a second atlas.

## Revision 461 combat-schema cleanup

Authored character-enemy combat data uses `attackType`, `damage`, `attackCooldown`, `spreadCount`, and `spreadAngle`. Runtime/editor loaders no longer accept the retired aliases `attackMode`, `attackDamage`, `projectileDamage`, `projectileCooldown`, `projectileVolleyCount`, or `projectileVolleyHalfAngle`; similarly, `meleeHitRadius` is no longer authored or represented in runtime enemy state. `attackRange` is projectile-only. Non-passive melee catalog definitions require positive `meleeHitRange`, and lunge-enabled definitions additionally require positive explicit `lungeTargetDist`. Internal runtime member names such as `attackMode` and `projectileVolleyCount` remain implementation details and are not authored schema aliases.

## Revision 463 Level Editor scoped resource residency

The Level Editor now has explicit ownership boundaries for expensive character presentation resources. The embedded production `RocketfrockRenderer` is authoritative for characters that exist in the current authored level. `editorRuntimeEnemyCharacterUrls(level)` derives its dependency set from `level.entities`, and changes to that set are serialized through `ensureCharacterProjects()`, which both loads missing projects and releases projects no longer needed. The overlay path consults `getRuntimeCharacterProjects()` first, so current-level enemies are not independently prepared a second time by the editor. The editor-local character-project map remains only for pre-renderer fallback and palette placement previews for a character not yet resident in the level.

`loadRuntimeCharacterProject()` accepts a caller-owned `preparedAtlasCache`. A cache entry owns the decoded source image plus immutable cropped frame canvases and their pixmap pyramids for one atlas manifest/pixmap mode. Each character project receives its own `atlasAssets` map pointing at those shared immutable frame assets. This is particularly important for character families whose distinct rigs/animations all use one large atlas. The renderer seeds later load batches from prepared atlas entries still owned by resident projects, so adding another variant can reuse the already-prepared atlas rather than decoding and pyramiding it again. Canvas-only hit-flash presentation derived from an identical source frame is also shared through a weak cache.

Full environment atlases in the editor-side atlas library are released on level switches when they are neither referenced by the new level nor the currently active palette atlas, provided the compact thumbnail catalog is available. The no-thumbnail compatibility fallback deliberately retains the full atlas library because those images are then the palette itself. The production renderer independently releases environment atlases through its existing `ensureEnvironmentAtlases()` contract. Sharing environment atlas image ownership between the authoring library and production renderer remains a possible later optimization; revision 463 does not couple those two subsystems.

## Revision 464 shared Level Editor environment-atlas sources

The authoring atlas library is now the decoded-source owner for environment and item atlases used by the Level Editor. `editorRuntimeEnvironmentAtlasSource()` resolves a renderer manifest URL back to the matching `state.atlases` record, and `createRenderer()` / `ensureEnvironmentAtlases()` accept an `environmentAtlasSourceResolver`. The renderer adapts that prepared source into an independent runtime atlas wrapper whose `image` references the same decoded `HTMLImageElement`, while `renderImage`, colour-treatment cache keys, WebGL texture residency, and release operations remain renderer-local. This removes the former second image decode/backing store without coupling mutable atlas records across the editor/renderer boundary.

`ensureEnvironmentAtlases()` also detects when a resolver now returns a different source image for an already-resident manifest, as happens after Reload atlas library, and rebinds the runtime wrapper before rebuilding renderer presentation caches. No process-global image cache is introduced, so the Revision 463 level-switch eviction rules continue to determine lifetime. Editor-only character previews additionally seed their prepared-atlas cache from current renderer projects, extending shared character-atlas frames to selected-but-unplaced variants.


## Revision 465 atlas-frame equivalence verifier

`reference/devel/verify_atlas_rect_pixels.py` is the pre-reforging content-equivalence boundary for atlas optimization. Atlas JSON `frames` are authoritative and usage is intentionally irrelevant, so defined-but-unused assets such as future human heads are protected exactly like active frames. Repacked coordinates may change, but frame identity and `w`/`h` remain stable. The verifier compares decoded RGBA crops in frame-local coordinates: alpha is exact everywhere and RGB is exact wherever either side has nonzero alpha. RGB under alpha zero is excluded from equivalence so transparent colour dilation can be introduced without altering rendered pixels.

The verifier treats authored source rectangles that cross a PNG boundary as logical fixed-size crops padded with transparent black. This preserves current frame-local geometry while allowing a future reforger to move the complete logical rectangle inside a compact rebuilt PNG. Directory mode discovers environment, character, and item atlas manifests recursively and requires mirrored manifest/frame sets.

## Revision 466 deterministic atlas reforger

`reference/devel/reforge_atlases.py` builds on the Revision 465 equivalence verifier and treats the manifest `frames` table as the complete preservation contract. Frame usage is never consulted, so currently unused authored assets remain available for future characters/items/levels. Frame names and dimensions are invariant; only atlas-space `x`/`y` placement changes.

The reforger collapses exact duplicate source rectangles and forms positive-area-overlap connected components before packing. Each overlap cluster preserves the source-relative position of every member, covering both deliberate containment (for example environment `_blend_1` frames) and legacy glancing overlaps without trying to infer author intent. Independent clusters can then be packed tightly. Source pixels outside every defined rectangle are omitted entirely.

Packed clusters receive a one-pixel alpha-zero gutter by default. A nearest-visible colour dilation copies RGB into transparent pixels while preserving alpha zero and leaving every visible pixel bit-exact. This protects linear GPU sampling without weakening the Revision 465 verifier. Layout selection is memory-monotonic by default: if the padded generic packing would require more decoded RGBA pixels than the source, the tool selects a cropped/source-relative fallback, so reforging never increases decoded atlas area merely to satisfy packing aesthetics.

The tool automatically verifies the rebuilt tree before reporting success, and in-place replacement is staged before authored files are overwritten. A full dry output of the current 59 atlas manifests preserves all 856 frames / 51,458,035 visible pixels and reduces aggregate decoded RGBA from about 463.1 MiB to 329.9 MiB (28.8%) without modifying the committed atlas resources in Revision 466.

## Revision 467 committed atlas-reforge rollout

Revision 467 applies the deterministic reforger to the authored production resources in `reference/resources/{atlases,characters,items}`. The committed atlas JSON/PNG pairs now use the compacted layouts produced by Revision 466's tool, while preserving every defined frame crop exactly under the Revision 465 verifier. Across the shipped atlas set, decoded RGBA area drops from about 463.1 MiB to 329.9 MiB (28.8%) with no frame-name or frame-dimension changes.

This rollout deliberately decouples gameplay/editor contracts from atlas-space placement. Runtime systems, authoring tools, and tests must treat manifest frame `x`/`y` as relocatable storage coordinates rather than authored semantic data. Behavioural contracts should instead depend on frame presence, dimensions, object wiring, and rendered output.

## Revision 468 moving-platform translation easing and pendulum motion

Moving-platform schema version 2 introduces an explicit motion dispatch while retaining the existing platform lifecycle and ownership model. `motionType: "translate"` keeps the established endpoint path. Its optional `easing` is one of `linear`, `easeIn`, `easeOut`, or `easeInOut`, sharing the quadratic interpolation functions used by character animation. Travel duration remains `distance / speed`; easing only remaps normalized progress. The 23 pre-existing shipped platform placements are migrated to explicit `translate` + `linear`, so their motion remains the legacy constant-speed path.

`motionType: "swing"` is a separate pendulum branch. It uses `angleAmplitude`, `initialAngle`, `swingPeriod`, and an unrestricted local-space `pivotX`/`pivotY`. The placement's ordinary rotation is the centre angle. Runtime derives a sinusoidal starting phase so positive and negative initial angles first move toward zero, while exactly zero starts counter-clockwise. Swing supports the shared initial/trigger delays and automatic, rider, or signal activation, but does not compose translation endpoints/easing with angular motion.

Both ports keep immutable initial swing geometry and rebuild the current platform-owned segments/polygons from that source at each angle, avoiding cumulative rotation drift. The browser collision index already treats moving-platform records as dynamic and recomputes their bounds at query time; native records remain in the existing dynamic moving-platform collision path. Supported player/enemy riders receive the point displacement produced by the angular transform but retain gravity-upright actor presentation. Because all atlas collision geometry follows the transform, existing damaging/killable lines provide hazardous pendulum edges without a second trap system.

## Revision 469 swept swing hazards and derived-resource integrity

Pendulum collision now retains the previous tick swing angle in addition to the current angle. Player surface-hazard evaluation adaptively samples the rigid angular sweep according to pivot radius and player size, testing both damaging/killable segments and filled polygons at intermediate poses. This closes the tunnelling hole where a fast blade could cross the player between fixed simulation poses. The same algorithm exists in browser and SDL runtimes. Closed damaging/killable atlas loops already materialize as `collisionPolygons`, so no parallel hazard-area schema was introduced.

Crush closing velocity for a swinging platform is evaluated at the candidate player/contact point by rotating that point back through the tick's angular delta; translating platforms continue to use their ordinary `lastDeltaX/Y`. In the editor, a shared initial-swing placement transform drives hit testing and selection, while path bounds sample the full swing envelope for viewport culling and Fit-to-Content.

Atlas reforging leaves thumbnail pixels valid but relocates source frame coordinates. Revision 469 refreshed that derived metadata and made stale source hashes/digest/frame rectangles release failures.

## Revision 470 phase-aware swing sweeps and palette-cache acceptance boundary

Pendulum hazard history is now represented by `swingSweepStartElapsed` plus `swingSweepDuration` for each simulation tick. Hazard sampling evaluates the same sinusoid as the platform motion using those elapsed times, so reversals at ±amplitude and period wrap remain visible to collision even when previous/current endpoint angles are equal. Arc-sampling density uses the sinusoid's maximum angular-speed bound and pivot radius. The player hazard rectangle is interpolated from its pre-platform-update pose to its end-of-tick pose at the same samples, producing a relative swept test rather than treating the actor as frozen at its final position. Browser and SDL implementations use the same state and equations.

The editor's swing path bounds include both the complete platform envelope and the external pivot itself, keeping the hinge/guide visible under Fit-to-Content. `refresh_palette_thumbnail_metadata.py` is now an intentionally narrow post-reforge operation: if source hashes changed it accepts only atlas JSON/PNG paths, requires `--original-resources` whose files match the cache's old hashes, and runs the atlas visible-pixel verifier before updating hashes or coordinates. Changes to `resources.json`, rigs, renderer code, or artwork cannot be certified by metadata refresh and require Palette Builder. `audit_resource_layout.mjs` independently enumerates `resources.json.assetAtlasIds` and requires every declared atlas source plus every authored frame entry in the palette cache. The ordinary release shard invokes all atlas Python regression suites, including curated frame-local visible-pixel fingerprints for identity-sensitive artwork.

## Revision 471 unified swing collision path

Pendulum movement now advances the actual runtime transform through adaptive angular substeps rather than moving directly to the tick endpoint and reconstructing hazards later. Each substep updates the sprite and platform-owned collision geometry, carries supported actors through the identical rigid transform, records damaging/killable contact at that live pose, and depenetrates non-riders from closed blockable collision polygons. This removes the previous mismatch between rider motion, hazard reconstruction, and solid collision, including extremum reversals where endpoint angles can match.

The substep count is derived from the maximum sinusoidal angular-speed bound and the farthest platform collision point from the pivot, with the allowed arc travel per sample tied to player body size. There is no fixed 512-sample ceiling, so very long-radius/fast but valid authored pendulums remain collision-safe instead of silently tunnelling once a cap is reached. Hazard contact is stored on the platform for the ordinary end-of-tick damage application, while normal final-pose surface checks still catch player-driven motion into the platform after the platform phase has finished.

Release-gate resume fingerprints now hash production atlas PNGs in the atlas, character, and item resource directories in addition to textual sources. This preserves resume efficiency without hashing large development-art folders, while ensuring PNG-only atlas edits rerun the curated frame-identity and atlas-tool tests.


## Revision 472 bounded pendulum interaction model

Swing authoring is intentionally bounded rather than treated as an arbitrary high-speed rigid-body simulator. `swingPeriod` normalizes to at least 2.0 seconds and the local `(pivotX,pivotY)` vector is limited to 800 px in both browser and native schemas. These constraints keep ordinary per-frame pendulum displacement small enough for the shared collision stepping model and replace Revision 471's deliberately pathological long-radius/ultra-fast stress case.

During each swing update the platform still evaluates the sinusoidal angular pose, but actor response between sampled poses is intentionally approximate: the short rigid-motion displacement at the actor location is applied as a translational push while the platform geometry receives the angular correction. A rider is collision-swept against world geometry other than its own platform, so walls/ceilings cannot be crossed by carry. Open solid segments, including the intended green walkable support lines and legacy yellow blockable lines, participate in intermediate contact/catch tests alongside closed blocking polygons. Non-rider character enemies receive the same short platform push/catch interaction so swing solids no longer ghost through them. This is gameplay collision, not exact continuous rotational dynamics.
