...
All cave containment and vertical-clearance queries must use the arbitrary polygon. `cavernVerticalRangeAt` finds every intersection interval at the requested X and selects the interval containing the relevant support or entity Y. Legacy top/bottom profile data may remain for old drafts and diagnostics, but it is not authoritative for contour caverns. The runtime remains generator-unaware and consumes the resulting cave-window points, atlas placements, entities, and world bounds through existing schemas.
...
`src/shared/level-generator-data.js` defines `GENERATED_POWER_UP_SPACING_PX = 2000`. The reward planner computes its target from mandatory-route distance and the existing density multiplier. The placement pass first distributes pickups across distinct safe supports, then may reuse sufficiently long non-door supports at normal reward-spacing intervals so Grand routes can satisfy the target without weakening endpoint, encounter, or geometry exclusions.
